...
char *file_head, *file_list, *file_direct; // temporary file string
...
int shuffle_merge(int num, char *input_file) {
    long i, j, k, l = 0;
    int fidcounter = 0;
    CREC *array;
    ...
    sprintf(tmp2, "%s%s.cooccur.shuf.bin", file_direct, input_file);
    fout = fopen(tmp2,"w");
...
int shuffle_by_chunks(char *input_file) {
    long i = 0, l = 0;
    int fidcounter = 0;
    char filename[MAX_STRING_LENGTH];
    CREC *array;
    ...
    array = malloc(sizeof(CREC) * array_size);
    char *tmp;
    tmp = malloc(sizeof(char) * MAX_STRING_LENGTH);
    sprintf(tmp, "%s%s.cooccur.bin", file_direct, input_file);
    fin = fopen(tmp,"rb");
    ...
    fclose(fid);
    free(array);
    return shuffle_merge(fidcounter + 1, input_file); // Merge and shuffle together temporary files
}
...
int shuffle_all() {
    ...
    while(fscanf(f_list, "%s\n", input_file) != EOF) {
        printf("Shuffling on time slice %d: %s\n", time_slice, input_file);
        shuffle_by_chunks(input_file);
        time_slice++;
    }
    fclose(f_list);
    free(input_file);
