...
real beta = 1.0; // Initial weight for previous W
real alpha = 0.75, x_max = 100.0; // Weighting function parameters, not extremely sensitive to corpus, though may need adjustment for very small or very large corpora
real *W, *gradsq, *cost, *prev_W; //prev_W is denotes the embeddings of the last time slice.
long long num_lines, *lines_per_thread, vocab_size;
char *vocab_file, *input_file, *save_W_file, *save_gradsq_file, *file_list, *file_direct, *vec_direct, *tmp_cat, *flag_file;
int *prev_vocab_flag; //vocab_flag denotes the words ever occurred in history.
...
void initialize_parameters_first_time() {//Init W and gradsq, and prev_W, and prev_vocab_flag
    long long a, b;
    vector_size++; // Temporarily increment to allocate space for bias
    
    /* Allocate space for word vectors and context word vectors, correspodning gradsq, and prev_vocab_flag */
    a = posix_memalign((void **)&W, 128, 2 * vocab_size * vector_size * sizeof(real)); // Might perform better than malloc
    if (W == NULL) {
        fprintf(stderr, "Error allocating memory for W\n");
        exit(1);
    }
    a = posix_memalign((void **)&prev_W, 128, 2 * vocab_size * vector_size * sizeof(real)); // Might perform better than malloc
    if (W == NULL) {
        fprintf(stderr, "Error allocating memory for prev_W\n");
        exit(1);
    }
    ...
    prev_vocab_flag = calloc(vocab_size, sizeof(int));
    for (b = 0; b < vector_size; b++) for (a = 0; a < 2 * vocab_size; a++) W[a * vector_size + b] = (rand() / (real)RAND_MAX - 0.5) / vector_size;
    for (b = 0; b < vector_size; b++) for (a = 0; a < 2 * vocab_size; a++) prev_W[a * vector_size + b] = (rand() / (real)RAND_MAX - 0.5) / vector_size;
...
void initialize_parameters() {//prev_W=W, gradsq=1
    long long a, b;
    vector_size++; // Temporarily increment to allocate space for bias   
    for (b = 0; b < vector_size; b++) for (a = 0; a < 2 * vocab_size; a++) prev_W[a * vector_size + b] = W[a * vector_size + b];
    for (b = 0; b < vector_size; b++) for (a = 0; a < 2 * vocab_size; a++) gradsq[a * vector_size + b] = 1.0;
    vector_size--;
}

void read_vocab_flag() {//read the vocab flag so that we know which word ever occurred in history and should influence future embeddings
    FILE *f_flag;
    int a;

    f_flag = fopen(flag_file, "r");
...
void *glove_thread(void *vid) {
    ...
    CREC cr;
    real diff, fdiff, temp1, temp2, hist_eff;
    FILE *fin;
    ...
    for(a = 0; a < lines_per_thread[id]; a++) {
        ...
        l1 = cr.word1 * (vector_size + 1); // cr word indices start at 0
        l2 = (cr.word2 + vocab_size) * (vector_size + 1); // shift by vocab_size to get separate vectors for context words
        
        /* Calculate cost, save diff for gradients */
        diff = 0;
        for(b = 0; b < vector_size; b++) diff += W[b + l1] * W[b + l2]; // dot product of word and context word vector
        diff += W[vector_size + l1] + W[vector_size + l2] - log(cr.val); // add separate bias for each word. If PMI is used, then no need for logarithm
        fdiff = (cr.val > x_max) ? diff : pow(cr.val / x_max, alpha) * diff; // multiply weighting function (f) with diff
        if(prev_vocab_flag[cr.word1] == 0) hist_eff = 0.0; // here
        else hist_eff = (cr.val > x_max) ? beta : pow(cr.val / x_max, alpha) * beta; // multiply weighting function (f) with beta, here
        cost[id] += 0.5 * fdiff * diff; // weighted squared error
        
        /* Adaptive gradient updates */
        fdiff *= eta; // for ease in calculating gradient
        hist_eff *= eta; // for ease in calculating gradient, here
        for(b = 0; b < vector_size; b++) {
            // weighted squared error, here
            cost[id] += 0.5 * hist_eff * (W[b + l1] - prev_W[b + l1]) * (W[b + l1] - prev_W[b + l1]); // here
            // learning rate times gradient for word vectors
            temp1 = fdiff * W[b + l2] + hist_eff * (W[b + l1] - prev_W[b + l1]); //here
            temp2 = fdiff * W[b + l1];
            // adaptive updates
...
/* Save params to file */
int save_params(int time_slice) {
    long long a, b;
...
/* Train embeddings of one time slice */
int train_one_time_slice(int time_slice) {
...
int train_dynamic() {
    FILE *f_list;
    int time_slice = 0;
    char com_flist[MAX_STRING_LENGTH + 1];
    strcpy(com_flist, file_direct);
    strcat(com_flist, file_list);
    ...
    if(verbose > 0) fprintf(stderr,"beta: %lf\n", beta);
    f_list = fopen(com_flist, "r");
    if(f_list == NULL) {fprintf(stderr, "Unable to open file list %s.\n",com_flist); return 1;}
    initialize_parameters_first_time();
    while(fscanf(f_list, "%s\n", tmp_cat) != EOF) {
        sprintf(input_file, "%s%s.cooccur.shuf.bin", file_direct, tmp_cat);
        sprintf(save_W_file, "%s%s-vectors", vec_direct, tmp_cat);
        printf("\nTraining model on time slice %d: %s\n", time_slice, input_file);
        train_one_time_slice(time_slice);
        sprintf(flag_file, "%s%s.vocab_flag", file_direct, tmp_cat);
        read_vocab_flag();
        time_slice++;
    }
    fclose(f_list);
...
int main(int argc, char **argv) {
    ...
    flag_file = malloc(sizeof(char) * MAX_STRING_LENGTH);
    ...
    vec_direct = malloc(sizeof(char) * MAX_STRING_LENGTH);
    tmp_cat = malloc(sizeof(char) * MAX_STRING_LENGTH);
    int result = 0;
    ...
    if (argc == 1) {
        ...
        printf("\t-beta <float>\n");
        printf("\t\tInitial weight for previous W in learning phase; default 1.0\n");
        printf("\t-alpha <float>\n");
        ...
        printf("\t\tList of names of cooccurrence data files; default file_list.txt\n");
        printf("\t-vocab-file <file>\n");
        printf("\t\tFile containing vocabulary (truncated unigram counts, produced by 'vocab_count'); default vocab.txt\n");
        printf("\t-vec-direct <file>\n");
        printf("\t\tDirectory for the output vectors; default ./vectors/\n");
        printf("\t-file-direct <file>\n");
        printf("\t\tDirectory for the input files; default ./data/\n");
    ...
    if ((i = find_arg((char *)"-vocab-file", argc, argv)) > 0) strcpy(tmp_cat, argv[i + 1]);
    else strcpy(tmp_cat, (char *)"vocab.txt");
    ...
    if ((i = find_arg((char *)"-vec-direct", argc, argv)) > 0) strcpy(vec_direct, argv[i + 1]);
    else strcpy(vec_direct, (char *)"./vectors/");
    
    vocab_size = 0;
    sprintf(vocab_file, "%s%s", file_direct, tmp_cat);
    fid = fopen(vocab_file, "r");
